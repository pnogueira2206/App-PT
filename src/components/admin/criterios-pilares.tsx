...
import { ATRIBUICOES_VAZIAS, definirPilar, obterAtribuicoesPilares, subscreverPilares } from "@/lib/pilares-store";
...
export function CriteriosPilares() {
  ...
  const categorizados = criteriosPontuaveis.filter((c) => atribuicoes[c.id]).length;

  return (
    <div className="mx-auto max-w-lg px-4 py-4">
      <h1 className="mb-1 text-lg font-semibold text-neutral-900">Critérios &amp; Pilares</h1>
      <p className="mb-4 text-sm text-neutral-600">
        Atribui a cada critério um dos 6 pilares do ensino eficaz. Isto vai alimentar a spider web de
                      ...
                      <div className="mt-2 flex flex-wrap gap-1.5">
                        {PILARES.map((p) => (
                          <button
                            key={p}
                            type="button"
                            onClick={() => definirPilar(c.id, atual === p ? null : p)}
                            className={`rounded-full border px-3 py-1.5 text-xs font-medium transition ${
                              atual === p ? "border-black bg-black text-white" : "border-neutral-300 text-neutral-700"
                            }`}
                          >
                            {p}
                          </button>
