...
export function VerAuditoria({ registos }: { registos: Registo[] }) {
  return (
    <div className="mx-auto max-w-lg px-4 py-4 md:max-w-2xl lg:max-w-3xl md:px-6 md:py-6">
      ...
      {registos.length === 0 ? (
        <p className="text-sm text-muted">Ainda não há registos.</p>
      ) : (
        <ul className="space-y-2">
          {registos.map((r) => (
            <li key={r.id} className="rounded-none border border-line px-3 py-2">
              ...
              <p className="mt-0.5 text-xs text-muted">{r.utilizador}</p>
              {r.detalhe && <p className="mt-0.5 text-xs text-dim">{r.detalhe}</p>}
            </li>
          ))}
        </ul>
      )}
